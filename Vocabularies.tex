...
(roughly: statements that are true for a wider term must be true
for a more specialised term, too).  This lets clients confidently perform
inferences.
...
used instead of the deprecated one.  This property MUST NOT be used with
non-deprecated subjects.
...
At the discretion of the vocabulary maintainer, the vocabulary is
...
reaches the status of Recommendation or Endorsed Note.  At that point,
all its terms become permanent, i.e., the \vocterm{ivoasem:preliminary}
triples on them are removed.  From then
on, the vocabulary is managed by the Semantics WG using the process defined in
the next section.
...
they choose to do so, simply send them via e-mail to the vocabulary
maintainer, for instance.  Authors are, however, encouraged to follow
the recommended procedure laid out in
Appendix~\ref{app:submitting-a-vep}.

The vocabulary maintainer will perform a
...
VEP-001, the second VEP-002, and so on.  The vocabulary maintainer
...
details are found in Appendix~\ref{app:curtech}).  The vocabulary
maintainer can extend the distribution as they see fit.  The
announcement in particular contains a copy of the VEP in question.

As soon as possible after the upload, the vocabulary maintainer adds
...
see Appendix~\ref{app:curtech}).  The vocabulary maintainer will
...
endorsment by the IVOA Executive Committee to become effective.
...
Executive Committee, the vocabulary maintainer has the
...
\appendix
\section{The 2022 IVOA Vocabulary Toolset (non-normative)}
\label{app:tools}
...
\label{app:vep-operations}

\subsection{Submitting a VEP}
\label{app:submitting-a-vep}

Authors of VEPs are encouraged to submit their vocabulary
...
for all VEP-introducing pull requests.  The vocabulary maintainer
will assign the running number when merging the pull
request.

\item Edit \verb|VEP-new.txt|, filling out the required fields.

\item Commit your changes locally:\\
\verb|git add VEP-new.txt && git commit -m "Adding VEP for ..."|
...
\verb|git commit -m "Accepting <term> as VEP-nnn" && git push|
...
\item Where the PR's branch belongs to you, delete the branch.
Locally check out the main branch again:\\
...
\subsection{Changes from PR-20220801}

Editorial changes after review.  Also, the repository now contains a
validator for the vocabulary repository.

\subsection{Changes from WD-20220516}
