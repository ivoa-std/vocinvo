...
vocabularies or use them to annotate instances documents will be able to
do so after reading just section~\ref{sect:withoutrdf}.  In particular, no
deeper understanding of RDF should be necessary.

Persons intending to participate vocabulary evolution should skim
...
VOTable 1.4 constrains the attributes of TIMESYS reference positions and time
...
\subsubsection{Extrnally Curated Vocabularies and VO Tooling}
\label{req:external}
...
The W3C's Resource Description Framework \citep{note:rdfprimer} is a
...
involved and in general requires special tooling, non-neglible
investment in understanding RDF, and non-trivial management of URIs and
prefix mappings.

To lower the bar for an adoption of IVOA vocabularies, they are given in
...
the vocabularies approved or discussed.  An individual vocabulary has a
URI like \url{http://ivoa.net/rdf/refposition}.  Dereferencing this URI
...
vocabulary, for instance (cf.~sect.~\ref{sect:usecases}):

\begin{compactitem}
\item Displaying labels and descriptions to users
\item Query expansion or similar exploitation of hierarchical
relationships
\item Validating annotated instances for the use of correct and current terms
\end{compactitem}
...
technical details), and at this point then new VEP is added there with
...
When introduing concepts, it is useful to consider a very simple
...
terms are disjunct (meaning: each thing in scope of the vocabulary
belongs to not more than one term's extension) and the terms cover the
entire scope (meaning: for each thing in the scope, there is at least
one term's extension that contains that thing): The terms are
equivalence classes over the vocabulary's scope.

When natural language is involved, this ideal generally is unreachable.
But when proposing a term and its definition, proposers should try to
make sure that 
...
true superset (in which case the other terms are narrower), or is true
...
cite them in the conventional author-title-year format.
...
cited in reference sections or scholarly works: such references will not
...
similar externally curated vocabularies).
...
new terms and introducing a native semantics model.
\end{itemize}
