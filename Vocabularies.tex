...
we use ``term'' primarily for ``the string behind the hash character in
...
requriements derived from the following use cases.
...
whereever feasible IVOA standards should build on IVOA-sanctioned,
...
in order to easily satisfy use case!\ref{uc:offlien} (or simply schemes
...
similar preferred label.  Definitions that contain the preferred label
itself are at least suspicious.  Definitions in non-English
languages are not permitted [again, this helps
requirement~\ref{req:mtm}].
...
are found that require those, this specifiation will be amended.  Until
...
sect.~\ref{sect:skosvoc-prop} apply.
...
The entrire new vocabulary gets approved as the document introducing it
...
If during discussion it is found that a VEP should be split, the
original VEP must be withdrawn and two new VEPs be prepared.
...
IsPreviousVersionOf allows marking up links to (typically
...
\begin{itemize}
\item Vocabulary: The URL or the vocabulary
\item Author: An e-mail address for a contact person for the author(s) of
the VEP.
\item Date: The date on which the VEP was posted.
\item Term: The identifier of the new term.
\item Action: one of \textit{Addition}, \textit{Deprecation}, or
\textit{Modification}.
\item Label: The English-language, human-readable label of the new term.
\item Description: The description that will come with the term.
\item Relationships: If applicable, relationships the new
term will have to existing terms, using the properties defined in
the present document.
\item Rationale: A discussion of use cases, the role of the new term in
the vocabulary, and the like.  In particular, an actual application of
the term should be given.
\end{itemize}

The items \textit{Term}, \textit{Action}, \textit{Label},
\textit{Description}, and \textit{Relationships} may be repeated if
multiple terms are affected by a VEP.  In \textit{Addition} VEPs, all items
except \textit{Relationships} are mandatory.

When \textit{Action} is \textit{Deprecation}, \textit{Label},
\textit{Description}, and \textit{Relationships} are optional but can be
...
When \textit{Action} is \textit{Modification}, \textit{Label},
\textit{Description}, and \textit{Relationships} give the proposed new
...
The Semantics working group maintains a web-based repository for VEPs
(see Appendix~\ref{app:curtech} for the technical details).  There, a
running number is assigned to the VEP at the time of the upload.  This
will be used to identify the VEP, where the running number is expanded
to three digits.  The first VEP will thus be VEP-001, the second
VEP-002, and so on.
...
(that may be updated during the process).
...
a term is preliminary is removed from the relationships of any new terms
in the vocabulary.  In the case of deprecation or modification VEPs, the
...
per sect.~2.13 of \citet{std:xmlsyntax}.
Both constraints are imposed to
...
\subsection{Changes from the Sneak Preview (Temporary)}

Until we put out the first WD, we list changes vs.~the last internal
working draft here for the benefit of persons following the early
development.  This will all be long gone by the time we get to REC.

\begin{itemize}
\item Now requiring RDF class and property vocabularies to have
tree-like hierarchies.

\item SKOS now uses wider rather than narrower so its direction matches
that of the RDF c/p flavours.

\item Added the common property ivoasem:vocflavour, mainly to let
clients figure out what vocabulary they're dealing with.

\item Added sample code to parse the vocabularies (``revovo'') both as a
proof of concept and as the seed of a validator.

\item Added XML advice (and an example) for the common properties.

\item Requiring absolute URIs in RDF/XML.

\item New use case/requirement on offline operation.

\item Now requiring RDF/XML to use typed node elements for our term
classes.

\item No longer linking ivoasem:deprecated and ivoasem:useInstead to
