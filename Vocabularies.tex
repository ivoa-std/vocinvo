...
Other VO standards can do with fewer normative constraints; using W3C
standards without the extra requirements laid down here is explicitly
encouraged where the use cases do not require the extra management and
definition effort, or where perhaps more complex structures (e.g., full
ontologies) must be employed.  An example for a direct use of SKOS
without adoption of the present document is the Simulation Data Model
SimDM \citep{2012ivoa.spec.0503L}, where several fields the values of
which are required to be \vocterm{skos:narrower} than certain top-level
...
We refer to classes and properties by CURIEs \citep{std:curie}.  
The prefixes in this
document correspond to the following URIs:
...
reaches the status of Recommendation or Endorsed Note.  From then
